  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <DashboardCard icon={FileText} label="Total Content" value={stats.total} color="primary" />
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/principal/pending" className="text-primary-600 hover:underline text-sm font-medium">View All</Link>
      </div>
        ...
        <div className="space-y-4">
          {pendingList.map(item => (
            <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm flex flex-col md:flex-row gap-4 items-start">
              <img src={item.fileUrl} alt="" className="w-full md:w-32 h-24 object-cover rounded-lg" />
              <div className="flex-1">
                <h3 className="font-semibold text-lg">{item.title}</h3>
                <p className="text-sm text-gray-500 mt-1">By {item.teacherName} • {item.subject}</p>
                <p className="text-xs text-gray-400 mt-1">Submitted: {formatDate(item.createdAt)}</p>
              </div>
              <div className="flex md:flex-col gap-2 w-full md:w-auto">
                <Button variant="primary" size="sm" className="flex-1 md:w-full" onClick={() => handleQuickApprove(item.id)}>
                  <Check className="w-4 h-4 mr-2" /> Approve
                </Button>
                <Button variant="danger" size="sm" className="flex-1 md:w-full" onClick={() => navigate('/principal/pending')}>
                  Review
                </Button>
              </div>
            </div>
